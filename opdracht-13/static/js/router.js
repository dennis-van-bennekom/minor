    ...
    var init = function () {
        ...
        routie({
            ...
            ':artist': (artist) => {
                ...
                var self = this;
                fetch(
                    `https://ws.audioscrobbler.com/2.0/?api_key=9112859bf77f54259252dc9bc48a8cb9&method=artist.getSimilar&format=json&limit=12&autocorrect=1&artist=${artist}`
                ).then(
            ...
            'detail/:artist': (artist) => {
                ...
                var self = this;
                fetch(
                    `https://ws.audioscrobbler.com/2.0/?api_key=9112859bf77f54259252dc9bc48a8cb9&method=artist.getInfo&format=json&autocorrect=1&artist=${artist}`
                ).then(
                    response => response.json()
                ).then(detail => {
                    ...
                    fetch(
                        `https://api.spotify.com/v1/search?type=artist&limit=1&q=${artist}`
                    ).then(
                        response => response.json()
                    ).then(data => {
